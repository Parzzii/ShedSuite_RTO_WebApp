ShedSuite RTO Web App - V7.25
=================================

V7.25 - Encrypted Company Manager / no XLSM runtime dependency
----------------------------------------------------------------
- Built from the stable V7.20.2 line; V7.21/V7.22 tenant auto-discovery is not used.
- Normal Delivery Certificate downloads no longer read Contract_Import.xlsm or Logininfo.
- Added a cross-platform encrypted vault at ~/.shedsuite_rto/company_vault.enc by default.
- Vault uses AES-256-GCM encryption; the encryption key is derived from one master password with scrypt.
- The master password is never stored. Enter it once per app/server session; credentials are then decrypted in RAM for fast use.
- Added Company Manager on the home/review pages.
- ShedSuite Accounts can be added, updated, tested and removed in the web app; saved passwords are never rendered back to the browser.
- Added an optional one-time Import legacy logins button so existing Logininfo credentials can be migrated from nearby Contract_Import.xlsm copies into the encrypted vault. After migration the XLSM file is not needed for normal operation.
- Added dynamic Company Registry: canonical company name, aliases, Store, Zone, Brand, Vendor, linked ShedSuite account and Delivery Certificate enable/disable.
- Saved company aliases tolerate common legal-name variations; conservative matching is used to avoid applying one company's settings to another.
- Dynamic company mappings override the V7.20 built-in mappings when the vault is unlocked. Existing V7.20 mappings remain a compatibility fallback for companies not yet moved into Company Manager.
- Delivery Certificate lookup is deterministic: linked company/account or the existing mapped login only. V7.25 never cycles through every saved ShedSuite tenant.
- A single ShedSuite account may be linked to multiple companies.
- Added dynamic model numbering: a company may reuse an existing model profile or define a custom prefix, minimum, maximum and zero-padding width. Custom series participate in next-model and sequential batch assignment.
- Dynamically configured companies can also appear in Inventory when they have a model series.
- Company Manager changes can requeue waiting/missing Delivery Certificate jobs in the current review session.
- Optional SHEDSUITE_VAULT_FILE can relocate the encrypted vault; the default location survives replacing the app ZIP with future versions.
- Reusable ShedSuite browser session/cookie state is stored inside the same encrypted vault, so sessions can be reused across app versions without leaving plaintext auth-state files.
