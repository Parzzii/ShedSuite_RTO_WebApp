...
V7.22.1 - Slow-order tenant discovery fix
------------------------------------------
- Fixes a false-negative in automatic ShedSuite tenant discovery that could reach the correct login, reject it too quickly, and continue checking every saved account.
- V7.22 used a fixed ~1.8 second post-navigation check. Some valid orders render their React order/file content later, so the correct tenant could look empty during that brief probe.
- Tenant probing now waits/polls for up to about 12 seconds, performs a best-effort network-idle wait, and checks the exact requested order route throughout the probe.
- Explicit login, access-denied, unauthorized, forbidden and order-not-found states still immediately reject a tenant.
- Positive evidence now includes the order ID, customer/order markers, file rows, customer surname + order markers, or a stable exact order route with substantive authenticated content and no negative state.
- This prevents the app from cycling through all saved logins when the correct tenant simply needs a few more seconds to render.
- Delivery Certificate capture itself is unchanged; once the tenant is confirmed, the existing long-wait certificate downloader runs normally.

V7.22 - In-app ShedSuite accounts + automatic tenant discovery
----------------------------------------------------------------
- Contract_Import.xlsm is no longer required for adding new ShedSuite credentials. The old Logininfo workbook remains a backward-compatible fallback only.
- Added a ShedSuite Accounts settings page directly in the web app. Add/update/remove ShedSuite login emails and passwords without editing Excel.
- On Windows, passwords saved by V7.22 are protected with Windows DPAPI for the current Windows user before they are written to the local account store.
- Passwords are never returned to the browser after saving and are never written into job folders/CSV/PDF output.
- Saved account emails are available in the manual login-mapping fallback selector.
- When an unmapped company/order appears, V7.22 automatically probes the saved ShedSuite accounts in bounded parallel background sessions instead of requiring the user to navigate tenants manually.
- A confidently discovered tenant is immediately saved as Company/Dealer -> ShedSuite login and reused on future jobs/versions.
- Accounts already authenticated during the batch reuse their Playwright browser context/session; discovery does not intentionally relaunch a new browser for every candidate order.
- If exactly one account contains the order, certificate retrieval continues automatically. If no account or multiple accounts can be identified confidently, the Review page keeps the manual login selector as a fallback.
- Adding a new account while a job is waiting automatically re-queues LOGIN NEEDED Delivery Certificate rows.
- Removing an in-app account also removes company/login mappings that pointed to that email, preventing stale mappings from silently selecting a deleted credential.
- Existing V7.21 saved company/login mappings continue to work, and existing legacy Logininfo credentials can be used alongside the new in-app account vault.
